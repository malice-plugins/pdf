...
class Elastic(object):
    def __init__(self, elastic_host):
        ...
        if not self.es.ping:
            raise ElasticsearchException(
                "[PING] cannot connect to host: {}".format(self.host)
            )
    ...
    def exists(self, id):
        if self.es.get(index="malice", doc_type="samples", id=id):
            return True
        return False
    ...
    def write(self, id, doc):
        if not self.exists(id):
            ...
            if not resp.get("result"):
                raise ElasticsearchException(
                    "[INDEX] failed to write doc with id: {}".format(id)
                )
        else:
            resp = self.es.update(
                index="malice", doc_type="samples", id=id, body=dict(doc=doc)
            )
            if not resp.get("result"):
                raise ElasticsearchException(
                    "[UPDATE] failed to update doc with id: {}".format(id)
                )
